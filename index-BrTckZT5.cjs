  ...
  return function(_a) {
    ...
    rulesToApply.forEach(function(rule) {
      ...
      for (var i = 0; i < matchers.length; i++) {
        if (tsubExports.matches(payload.obj, matchers[i])) {
          payload.obj = tsubExports.transform(payload.obj, transformers[i]);
          if (payload.obj === null) {
            return next(null);
          }
        }
